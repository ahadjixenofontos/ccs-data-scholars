...
author: "Athina Hadjixenofontos, Ph.D, Guanhua Chen"
date: "6/1/2017"
...
## Mapping a map of the United States of America

So let's put the Miami-Dade libraries on a map! 

Let's start with writing the code to draw a map of the world. The dataset you chose may not map to Miami-Dade. Instead, your data may map to the coast of west Africa and western Europe, or to the west coast of the United States. 

**IMPORTANT:** After we draw the world map, we will zoom in to the area that concerns our datasets. I'll be using Miami-Dade as my zooming-in area because that's what's in the demonstration data, but you should zoom-in to the areas that are in YOUR chosen dataset. 
...
world_plot <- ggplot(data = world) +
                geom_polygon(aes(x = long, 
                                 ...
                                 fill = region, 
                                 group = group)) +
                coord_fixed(1.2) + # this makes sure the map isn't too stretched
                guides(fill = FALSE) # this removes the legend 
world_plot # this calls the plot named "world_plot" to appear on the screen
```

Woohoo! Good job. 

#### Let's zoom-in on Florida, so we can see Miami-Dade county. 
