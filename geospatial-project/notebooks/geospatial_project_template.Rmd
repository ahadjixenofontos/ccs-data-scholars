---
title: "Geospatial Data Project - Analysis Template"
author: "Athina Hadjixenofontos, Ph.D and Guanhua Chen"
date: "6/1/2017"
output: html_notebook
---

How to use: Please open this R Markdown document in RStudio, so you can adapt it for your own analysis. See <http://rmarkdown.rstudio.com> for more on R Markdown. 
...
The gray block below contains R code. You'll be running the code, seeing what it does, changing it, testing that your changes are doing what you expected they would do, all in these gray boxes. The only thing you **won't** need to change are the first and last lines that contain the backticks `. 
...
```{r}
libraries <- read.csv("~/ccs-data-scholars/ccs-data-scholars/geospatial-project/datasets/libraries.csv", header = TRUE)
```
...
```{r}
states <- map_data("state") # load the data for all states in the USA
counties <- map_data("county") # load the data for all counties in the USA

florida <- states[states$region == "florida",] # from all states, select only Florida 
florida_counties <- counties[counties$region == "florida",] # from all counties, select only Florida counties
```
...
```{r}
ggplot() + 
  geom_polygon(data = florida, # geom_polygon to connect the dots, using the florida data
...
Now that we've figured out the code for plotting Florida, we can also save the plot in a variable, so we can access it later. 

```{r}
florida_map <- ggplot() + # save the plot in a variable named "florida_map"
  geom_polygon(data = florida, 
               aes(x = long, 
                   y = lat, 
                   group = group)) +
  geom_polygon(data = florida_counties,   # plot Florida counties on top
               aes(x = long, 
                   y = lat, 
                   group = group), 
...
florida_map # call the plot named "florida_map" to appear on the screen
```
...
Awesome! Now, follow the plotting examples above to produce a map of the state represented in your dataset, complete with county lines. 
...
First, let's remind ourselves what we have in our dataset by taking another look. 
...
libraries_map <- ggplot() +        # I chose the map name to be "libraries_map"
  geom_polygon(data = florida,        # draw the coastline of Florida
               aes(x = long, 
                   y = lat, 
                   group = group)) +
  geom_polygon(data = florida_counties,     # draw the Florida county lines
               aes(x = long, 
                   y = lat, 
                   group = group), 
               fill = "gray", 
...
## Adding street maps to the plot

The `ggmap` package is an incredible library that allows us to download maps from Google or from a project called Open Street Maps to use in our plots. This means you can add streets to the plot! 
