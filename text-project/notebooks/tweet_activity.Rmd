...
The two celebrities competing for the spot will be Kim Kardashian and Donald Trump. Lets load the datasets and take a look at what information our dataframes contain

```{r}

Kim_tweets <- read.csv("/Users/asiataylor/Documents/Data Scholars/KimKardashianTweets.csv", header=TRUE)
Trump_tweets <- read.csv("/Users/asiataylor/Documents/Data Scholars/DonaldTrumpTweets2.csv", header=TRUE )
...
```
Who a user mentions in their tweets can reveal some information about that user. Using grep, lets extract all of the people that Kim and Trump mention in this dataset. 
```{r}
...
```
Now that we have gathered all of the mentions and their counts, we can use this information to attempt to draw some insigths. Let's do some basic calculations. 
```{r}
